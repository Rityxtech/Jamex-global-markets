...
const LS_LAST_READ_KEY = 'jamex_livechat_last_read';

function getLastReadTime(): string | null {
  return localStorage.getItem(LS_LAST_READ_KEY);
}

function setLastReadTime(time: string) {
  localStorage.setItem(LS_LAST_READ_KEY, time);
}
...
export default function LiveChatWidget() {
    ...
    if (userId) {
      ...
      if (!error && data) {
        ...
        const unreadCount = data.filter(m => {
          ...
          if (m.read_by_user) return false;
  ...
  useEffect(() => {
    if (open && messages.length > 0) {
      ...
      if (unreadAdmin.length > 0) {
        ...
        supabase.from('livechat_messages').update({ read_by_user: true }).in('id', ids)
          .then(({ error }) => {
            if (error) {
              console.error('[LiveChat] Failed to mark messages as read:', error);
            }
            setLastReadTime(now);
            setUnread(0);
            fetchMessages();
  ...
  return (
    <div className="fixed bottom-20 right-4 z-[100] flex flex-col items-end md:bottom-4">
      ...
      {/* Mobile Fullscreen Right Drawer */}
      ...
      {open && (
        <div className="hidden md:flex mb-3 w-[340px] max-w-[90vw] h-[480px] bg-surface-container-lowest rounded-2xl shadow-2xl border border-outline-variant/30 flex-col overflow-hidden z-[100] relative">
          {/* Header */}
          <div className="bg-gradient-to-r from-[#2563eb] to-[#b4c5ff] px-4 py-3 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2">
              <span className="material-symbols-outlined text-white text-[20px]">chat</span>
      ...
      <button
        ...
        className={`w-14 h-14 rounded-full bg-gradient-to-br from-[#2563eb] to-[#b4c5ff] shadow-[0_8px_30px_rgba(37,99,235,0.4)] hover:shadow-[0_12px_40px_rgba(37,99,235,0.6)] hover:scale-105 transition-all items-center justify-center relative ${
